<template>
  ...
  <v-dialog v-model="deleteDialog" max-width="500px">
    <v-card>
      ...
      <v-card-actions>
        <v-btn color="blue" text @click="deleteDialog = false">Abbrechen</v-btn>
        <v-btn color="red" text @click="deleteKunde">Löschen</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
  ...
  <v-snackbar v-model="snackbar" timeout="3000" top right>
    ...
    <v-btn color="pink" text @click="snackbar = false">Schließen</v-btn>
  </v-snackbar>
</template>
...
export default {
  ...
  methods: {
    ...
    confirmDelete(kunde) {
      this.currentDeleteId = kunde.id;
      this.deleteDialog = true;
    },

    deleteKunde() {
      axios.delete(`http://localhost:8000/api/kunden/${this.currentDeleteId}`)
        .then(() => {
          // Filter out the deleted customer from the list
          this.kunden = this.kunden.filter(kunde => kunde.id !== this.currentDeleteId);
          this.filteredKunden = this.kunden;  // Re-filter the list
          this.snackbar = true;  // Show success message
        })
        .catch(error => {
